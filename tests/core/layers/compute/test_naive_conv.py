...
from tests.core.layers.compute.test_conv import assert_conv
from ...utils import set_seed, generate_random_tensor, tensor_to_cupy, cupy_to_tensor


def assert_naive_conv(batch_size: int,
                in_channels: int,
                out_channels: int,
                img_size: int = 8,
                *,
    ...
    conv_cp = NaiveConv(in_channels, out_channels, kernel_size, stride=stride, padding=padding)
...
def test_naive_conv():
    assert_naive_conv(batch_size=2, in_channels=3, out_channels=4, kernel_size=3)

def test_naive_conv_stride():
    assert_naive_conv(batch_size=2, in_channels=3, out_channels=4, kernel_size=3, stride=2)

def test_naive_conv_padding():
    assert_naive_conv(batch_size=2, in_channels=3, out_channels=4, kernel_size=3, padding=0)